...
const UserDashboard = () => {
    ...
    const SLOT_MIN_MINUTES = 9 * 60 + 30; // 09:30
    const SLOT_MAX_MINUTES = 20 * 60;     // 20:00

    const isSlotInCalendarRange = (slot) => {
        if (!slot?.start_time) return false;
        const startDate = new Date(slot.start_time);
        const minutes = startDate.getHours() * 60 + startDate.getMinutes();
        // même fenêtre horaire que le calendrier
        if (minutes < SLOT_MIN_MINUTES || minutes >= SLOT_MAX_MINUTES) return false;
        // s'aligner sur un pas de 45 min à partir de 09h30 (09h30, 10h15, 11h00, ...)
        const delta = minutes - SLOT_MIN_MINUTES;
        return delta % SLOT_DURATION_MINUTES === 0;
    };
    ...
    const getSlotEndDate = (slot) => {
        ...
        const startDate = new Date(slot.start_time);
        return new Date(startDate.getTime() + SLOT_DURATION_MINUTES * 60000);
    };

    // Filtrer uniquement les créneaux disponibles (non pris) et alignés sur les horaires du calendrier admin
    const availableSlots = slots
        .filter(slot => !slot.taken)
        .filter(isSlotInCalendarRange);
...
const fetchSlots = async () => {
    try {
        ...
        const response = await axios.get(`${API_URL}/slots?start_date=${today}&end_date=${endDate}`, {
...
const handleCancelAppointment = async (appointmentId) => {
    ...
    const toggleNotifications = async () => {
        try {
            await axios.put(`${API_URL}/users/notifications`, {
                                        ...
                                        <li key={slot.id} className="slot-item">
                                            <div className="slot-details">
                                                ...
                                                <span className="slot-time">
                                                    {new Date(slot.start_time).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })} - {getSlotEndDate(slot)?.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}
                                                </span>
                                            </div>
                                            <input
                                                type="checkbox"
